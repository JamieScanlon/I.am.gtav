...
local function DrawMenu()
    ...
    local msg = ""
        ...
        if WarMenu.Button(k,msg) then TriggerEvent("emotes:playanimation", k) end
    end
end
...
local function DrawFavorites()
    ...
    for k,v in spairs(favorites, function(t, a, b) return string.lower(tostring(t[a])) < string.lower(tostring(t[b])) end) do
        if WarMenu.Button(v) then TriggerEvent("emotes:playanimation", v) end
    end
end
        ...
        if pAnimation == "Cancel Emote" then
            ...
            if not isCuffed then
                if GetEntityModel(PlayerPedId()) == GetHashKey("a_c_chop") then
                    TriggerEvent("emotes:playanimation", dogEmote[pKey])
                else
                    TriggerEvent("emotes:playanimation", pAnimation)
                end
            end
        end
    end
